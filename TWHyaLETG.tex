...
%1000 HST orbits to observe the UV emission in young stars. ULLYSSES
...
incomplete. 
% A hot wind is the most promising candidate for this
% missing component. 
...
% TODO: Check that simulatenous/contemporaneous and time limits are consistent
...
\caption{\label{fig:CIVsketch} \emph{left:} Structure of the accretion
  ...
  %The yellow spectrum is from an undisturbed photosphere, the orange one from the heated photosphere.
...
Disk models indicate that X-rays can regulate the chemistry in the disk mid-plane where most of the mass is and planets form (e.g.\ \bibentry{Cleeves_2017}). However, these models are uncertain because the spatial origin and the time behavior of the X-ray flux and spectrum are not well known.
...
respectively (\bibentry{Ardila_2013}). However, only few systems
...
(Fig.~\ref{fig:UV2}; \bibentry{Robinson_2019}) presumably due to variations in the
...
% (top axes gives temperature derived from $(f+i)/r$ ratio). 
Without absorption,
